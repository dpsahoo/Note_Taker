...
def create_app(test_config=None):
    ...
    def sign_up():
        if request.method == 'POST':
            ...
            # If user entered information is OK, create a new User object and add it to the database
            if error is None:
    ...
    # Routes for 'notes' handling
    @app.route('/notes')        # Lists all notes for the user
    @require_login
    def note_index():
        return render_template('note_index.html', notes=g.user.notes)

    @app.route('/notes/new', methods=('GET', 'POST'))    # Create a new note
    @require_login
    def note_create():
        if request.method == 'POST':
